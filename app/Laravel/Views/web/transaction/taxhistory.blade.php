...
<section class="px-120 pt-110 pb-80 gray-light-bg">
    <div class="container">
         ...
         <div class="row flex-row items-center px-4">
            ...
            <a href="{{route('web.transaction.create')}}?type=ctc" class="custom-btn badge-primary-2 text-white " style="float: right;margin-left: auto;">E-Community Tax Certificate</a>
         </div>
        ...
        <div class="card">
          <table class="table table-responsive table-striped table-wrap" style="table-layout: fixed;">
              ...
              <tbody>
                @forelse($tax_transactions as $transaction)
                <tr class="text-center table-font">
                  <td>{{$transaction->customer->full_name}}</th>
                  <td>{{$transaction->transac_type->name}}<br><a href="{{route('web.transaction.ctc_show',[$transaction->id])}}">{{$transaction->code}}</a></th>
                  <td>
