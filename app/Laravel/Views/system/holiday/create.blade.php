    ...
    <div class="card-body">
      ...
      <form class="create-form" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control {{$errors->first('name') ? 'is-invalid' : NULL}}" id="input_title" name="name" placeholder="Department Name" value="{{old('name')}}">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-control {{$errors->first('date') ? 'is-invalid' : NULL}}" id="input_date" name="date"  value="{{old('date')}}">
          @if($errors->first('date'))
          <p class="mt-1 text-danger">{!!$errors->first('date')!!}</p>
          @endif
        </div>

        <button type="submit" class="btn btn-primary mr-2">Create Record</button>
        <a href="{{route('system.department.index')}}" class="btn btn-light">Return to Holiday List</a>
      </form>
    </div>
  </div>
</div>
